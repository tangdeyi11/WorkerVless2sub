...
let addresses = [
	//'47.128.173.23:443#SG',
	'47.130.47.224:443#SG',
	'45.77.36.40:443#SG',
	'158.178.238.239:443#SG',
	'52.221.204.184:443#SG',
	'168.138.162.187:443#SG',
	'178.128.80.43:443#SG',
	'152.69.222.9:443#SG',
	'167.99.73.22:443#SG',
	'143.198.213.197:443#SG',
	'188.166.255.195:443#SG',
    '185.114.78.230:443#SG',
    '146.190.111.165:443#SG', 
    '51.79.158.126:443#SG',
	//'192.243.127.224:443#JP',//AMAZON-02 ASN16509 GOOD  CUC
	'23.106.140.129:443#JP',
	'133.167.108.98:443#JP',
	'152.32.168.249:443#HK',
	'156.234.17.144:443#HK',
	'103.192.178.12:443#HK',
	'103.225.198.7:443#HK',
	//'128.1.132.82:443#HK',  // HK PROXY OK
	//'128.1.131.152:443#HK', // HK PROXY OK
	//'211.114.182.209:25565#KR',
	'104.17.154.227:443#US',
	'104.20.6.176:443#US',
	'104.16.126.145:443#US',
	'104.19.229.12:443#US',
	'104.19.218.222:443#US',
	'104.16.127.28:443#US',
	'104.19.251.253:443#US',
];
...
export default {
	async fetch (request, env) {
		...
		*/
		...
		if (!userAgent.includes('subconverter') && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
		...
		} else if ( (userAgent.includes('clash') || (format === 'clash' && !userAgent.includes('subconverter')) ) && !userAgent.includes('nekobox') && !userAgent.includes('cf-workers-sub') && !userAgent.includes('cf-workers-edgetunnel/cmliu')) {
			subconverterUrl = `https://${subconverter}/sub?` +
			`target=clash` +
			`&url=${encodeURIComponent(subconverterUrl)}` +
			`&insert=false` +
			`&config=${encodeURIComponent(subconfig)}` +
			`&emoji=true&list=false` +
			`&tfo=false&scv=true&fdn=false` +
			`&sort=false&new_name=true`;
		} else if ( (userAgent.includes('sing-box') || userAgent.includes('singbox') || (format === 'singbox' && !userAgent.includes('subconverter')) ) && !userAgent.includes('cf-workers-sub') && !userAgent.includes('cf-workers-edgetunnel/cmliu')){
			subconverterUrl = `https://${subconverter}/sub?` +
			`target=singbox` +
			`&url=${encodeURIComponent(subconverterUrl)}` +
			`&insert=false` +
			...
			if (协议类型 == 'Trojan' && (userAgent.includes('surge') || (format === 'surge' && !userAgent.includes('subconverter')) ) && !userAgent.includes('cf-workers-sub')) {
				...
				subconverterUrl =  `https://${subconverter}/sub?` +
				`target=surge&ver=4` +
				`&url=${encodeURIComponent(TrojanLinksJ8)}` +
				`&insert=false` +
				`&config=${encodeURIComponent(subconfig)}` +
				`&emoji=true&list=false` +
				`&xudp=false&udp=false` +
...
// surge函数处理配置内容，将其中的"trojan"相关内容转换为"ws"类型配置
function surge(content, url) {
    let 每行内容; // 定义一个变量用于存储拆分后的内容
    // 判断换行符类型并将内容按行拆分
    if (content.includes('\r\n')) {
        每行内容 = content.split('\r\n'); // 使用\r\n拆分
    } else {
    ...
    for (let x of 每行内容) {
        ...
        if (x.includes('= trojan,')) {
            ...
            const 备改内容 = `skip-cert-verify=true, tfo=false, udp-relay=false`; // 旧的配置内容
            const 正确内容 = `skip-cert-verify=true, ws=true, ws-path=/?ed=2560, ws-headers=Host:"${host}", tfo=false, udp-relay=false`; // 新的配置内容
            // 替换旧配置为新配置并去掉多余的括号
    ...
    return 输出内容; // 返回最终处理后的内容
}

// 根据匹配条件从代理列表中随机选择一个代理
function getRandomProxyByMatch(CC, socks5Data) {
    // 将匹配字符串转换为小写，进行不区分大小写的匹配
...
async function MD5MD5(text) {
    ...
    // 第二轮MD5计算，取第一轮结果的某些部分
    const secondPass = await crypto.subtle.digest('MD5', encoder.encode(firstHex.slice(7, 27)));
    const secondPassArray = Array.from(new Uint8Array(secondPass));
    const secondHex = secondPassArray.map(b => b.toString(16).padStart(2, '0')).join('');
  
    return secondHex.toLowerCase(); // 返回第二轮的MD5值
}
...
function generateFakeInfo(content, userID, hostName) {
    content = content.replace(new RegExp(userID, 'g'), fakeUserID) // 将真实的用户ID替换为伪造的
                     .replace(new RegExp(hostName, 'g'), fakeHostName); // 将真实的主机名替换为伪造的
    return content;
}
...
function isValidIPv4(address) {
    ...
    return ipv4Regex.test(address); // 使用正则表达式检查IPv4地址格式是否正确
}
